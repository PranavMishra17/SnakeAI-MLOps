...
bool Menu::loadBackgroundImage(const std::string& imagePath) {
    if (m_backgroundImageTexture.loadFromFile(imagePath)) {
        ...
        // SFML 3: Create sprite with texture
...
void Menu::setupBackgroundImage(sf::RenderWindow& window) {
    ...
    // SFML 3: Use sf::Vector2f for scale
    ...
    // SFML 3: Use sf::Vector2f for position
...
void Menu::setupEnhancedLayout(sf::RenderWindow& window) {
    ...
    // SFML 3: Text constructor with font
    m_title = std::make_unique<sf::Text>("SnakeAI-MLOps", m_font, 56);
...
void Menu::setupMainMenu() {
    ...
    for (size_t i = 0; i < m_mainItems.size(); ++i) {
        ...
        // SFML 3: Text constructor with font and size
        item.displayText = std::make_unique<sf::Text>(item.text, m_font, 26);
...
void Menu::setupPlayModeMenu() {
    ...
    for (size_t i = 0; i < m_playModeItems.size(); ++i) {
        ...
        // Mode title - SFML 3 constructor
        item.displayText = std::make_unique<sf::Text>(item.text, m_font, 24);
        item.displayText->setFillColor(sf::Color(47, 79, 47));
        item.displayText->setStyle(sf::Text::Bold);
        ...
        // Enhanced explanation text - SFML 3 constructor
        item.explanationText = std::make_unique<sf::Text>(item.explanation, m_font, 15);
